...
| **S3**（1 天）✅ 已完成 | 全局变量/顶层语句完善（T4）+ 端到端三文件用例（T7）+ 入口参数（T8） | ✅ T4/T7/T8 通过 |
| **S4**（可选） | static 文件级作用域、头文件去重优化、CLI（`python program.py a.c b.c`） | 后续路线 |

**S3 实现注记**：
- `test/multi/` 新增 6 夹具：gv_lib/gv_main（全局变量 init 调跨文件函数）、e2e_lib/util/main（三文件综合）、args_main（带参入口）
- T7 端到端三文件 `main()=116`：跨文件 enum（GREEN）+ struct 返回值 + 值拷贝 + 同类型静默去重
- T8 入口参数 `run(3,4) → 7`：main 形参带类型绑定
- **顺带修复**：真实解析器把 `typedef enum Color {...} Color;` 的 `Typedef.type` 包装成 `TypeDecl(Enum)`（struct typedef 同理）——`ExeTypedef` 的枚举分支需先解包 TypeDecl，否则枚举常量不会注入作用域（T7 初版暴露，修复后 GREEN 可用）
