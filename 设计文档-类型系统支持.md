...
### 3.2 M1 typedef（约 1 天）

- [ ] `TypeRegistry.register_typedef/resolve_typedef`（链式 + 环检测）
- [ ] 重构 `ExeTypedef`：`type_of_decl` 解析 → 注册别名（不再占位 None）
- [ ] `ExeDecl` 类型解析接入 `g_types.resolve`（未知类型报 `UndefinedType`）
- **测试**：`typedef int MyInt; MyInt x = 5;`；`typedef MyInt MyInt2; MyInt2 y;`（链式）；环检测报错；`typedef struct {int a;} S; S s;`（内联匿名 struct）
- **验收**：`get_type('x') is IntType`；`sizeof` 走类型路径后可算出 4

### 3.2 M2 enum（约 1 天）

- [ ] `EnumType(constants)`；`ExeEnum` 注册标签 + 常量注入（带 IntType）
- [ ] `g_types.resolve` 支持 `['enum','Color']` 形式
- **测试**：默认递增/显式值/引用前一常量/`typedef enum {...} Color`/`enum Color c; c = GREEN;`
- **验收**：`g_scope.get('RED')==0`（现有测试保持）；`enum Color` 变量类型为 `EnumType`

### 3.3 M3 struct（约 5 天，核心）

- [ ] D1 `compute_struct_layout`（含对齐、空 struct、GNU `aligned` 覆盖）
- [ ] D2 `StructValue`（默认值/`get`/`set`/`copy` 深拷贝）
- [ ] D3 `ExeStruct`/`ExeStructExt` 重构：布局 + 注册（**移除成员污染**，回归 test 更新）
- [ ] D4 `ExeDecl` 实例化：`coerce_to_type`（InitList 顺序填充 / StructValue 拷贝 / 默认值）
- [ ] D5 `ExeStructRef` 支持 `.` 成员访问（读 + 写）
- [ ] D6 嵌套 struct + struct 数组 + 复合字面量 `(struct S){...}`
- [ ] D7 `->` 宽松退化（struct 值上等价 `.`）；正式指针支持对接 MEM-1
- **测试**（含端到端 `test/main.c`）：
  - `struct Point {int x,y;} p = {1,2}; p.y = 5;`
  - `struct Rect { struct Point tl, br; } r = {{0,0},{10,10}}; r.br.x`
  - `struct S a = b; a.x=1;` 不改变 `b`（值拷贝）
  - `struct S arr[2] = {{1},{2}}; arr[1].x`
  - 位域占位 `struct {int a:3; int b:5;}`
  - GNU：`struct S s __attribute__((aligned(16)));` 对齐生效
- **验收**：`sizeof(struct Point)==8`；成员不进入全局作用域（修复 P1）；现有 `test_struct_union` 需按新语义更新

### 3.4 M4 union（约 2 天）

- [ ] `compute_union_layout`；`UnionValue`（活跃成员模型）
- [ ] `ExeUnion` 注册；`ExeStructRef` 支持 union 成员访问
- **测试**：`union U {char c; int i;}; U u; u.i=0x41;`（活跃成员读）；sizeof(union)==max 成员对齐后大小
- **验收**：共享存储语义正确（读未活跃成员返回 0/告警，逐字节 reinterpret 标注为 MEM-1 前置）

### 3.5 M5 集成增强（约 4 天）

- [ ] `infer_type` 表达式类型推导（Constant/ID/BinaryOp/Cast/StructRef/ArrayRef/FuncCall）
- [ ] `sizeof`/`__alignof__` 走类型路径（**修 BUG-1/2**：`sizeof(x++)` 不求值）
- [ ] struct 值传递：函数参数/返回值 `copy()`；参数类型检查
- [ ] 位域真实打包（可选）；指定初始化器完善（`[2]=v`）
- [ ] 类型化 Cast 增强（struct↔指针、enum↔int）
- **测试**：`sizeof(x++)` 无副作用；`sizeof(struct S)`；函数传 struct 改形参不影响实参；`enum` 与 `int` 互转
- **验收**：TODO.txt BUG-1/2 关闭；MEM-3/TYPE-1 标记完成
